...
export function Stats() {
  ...
  // --- Calendar heatmap ---
  const heat = state.history.map((c) => {
    let bg: string;
  ...
  return (
    <div style={{ padding: '28px var(--pad) 48px', animation: 'rvFloat 0.4s ease both' }}>
      ...
      <Card title="Calendar">
        <div style={{ display: 'grid', gridTemplateRows: 'repeat(7, 13px)', gridAutoFlow: 'column', gridAutoColumns: '13px', gap: 4, overflowX: 'auto', paddingBottom: 4 }}>
          {heat.map((cell, i) => (
            <div key={i} title={cell.title} style={{ width: 13, height: 13, borderRadius: 3, background: cell.bg }} />
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 250, background: 'oklch(0.99 0 0)', border: '2px solid oklch(0.9 0 0)', borderRadius: 18, padding: '20px 22px', marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 110, borderBottom: `1px solid ${BASELINE}` }}>
            {ivBuckets.map((b) => (
              <div key={b.label} style={{ flex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                <div title={`${b.count} cards`} style={{ width: '100%', height: `${(b.count / ivMax) * 100}%`, minHeight: 3, background: 'var(--accent)' }} />
              </div>
            ))}
          </div>
        ...
        <div style={{ flex: 1, minWidth: 250, background: 'oklch(0.99 0 0)', border: '2px solid oklch(0.9 0 0)', borderRadius: 18, padding: '20px 22px', marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 110, borderBottom: `1px solid ${BASELINE}` }}>
            {added.map((c, i) => (
              <div key={i} style={{ flex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                <div title={`${c} added`} style={{ width: '100%', height: `${(c / addedMax) * 100}%`, minHeight: 2, background: tokens.newBlue }} />
              </div>
            ))}
          </div>
